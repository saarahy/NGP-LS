...
with open(direccion) as spambase:
    ...
    for row in spamReader:
          datos_x.append(float(row[0]))
          datos_y.append(float(row[1]))
print datos_x
print datos_y

direccion2="./data_pierrick/data_%d.txt"%(id_number)
...
def f1(x0, soluciones): #neat-GP-LS neat-cx
...
line1,=plt.plot(d_x,datos_y, marker='o',label='Original data')
#strng=
line2,=plt.plot(d_x, f1(d_x, soluciones), label='neat-GP-LS neat-cx (%s)' % aptitud[0])
...
plt.annotate(aptitud[0], xy=(d_x[-1], f1(d_x[-1], soluciones)), color=line2.get_color(), size=13)
...
plt.title('patient1.csv', size=20)
